---
title: How to Generate BUFR Descriptor Files
---
...
There is a GrADS interface for BUFR, which means that BUFR data can be read directly in their native format and are handled as a GrADS station data set with all the associated display an analysis capabilities. GrADS requires a specially-formatted descriptor file to read BUFR data; the output from [`bufrscan`](gradutilbufrscan.html), an external GrADS utility, is used to compose the descriptor file.
...
| Entry | Description |
|---|---|
| DSET | This entry points to the BUFR data file. It is not currently recommended to use templating with BUFR data. (See Note on Templating below) |
...
The first step in generating a BUFR descriptor file is figuring out the X,Y values for the data and metadata variables that GrADS requires. Begin by perusing the header output from [`bufrscan`](gradutilbufrscan.html) looking primarily at the numeric elements. Here is a link to some [example header output](_downloads/bufr.sample.headers) -- a subset of this is given below. 
